...
NO_DATA: int = 255
OTHER_DATA: int = 255#10 #*MW should we just be using NO_DATA, supplying this via function, or what? i think we should just have input nodata (whcih can be determined from xr, and output nodata)
FILL_DATA: int = 10 #*MW wb this?
BURN_AREA_VALUE: int = 15 #*MW shouldn't this be variable ? eg 15 won't work for 3sl
STRIP_FILE_PATH_KEY: str = 'landcover'
MASK_FILE_PATH_KEY: str = 'cloudmask'
GRID_CELL_NAME_PRE_STR: str = 'CAS.M1BS'
DIM: str = "time"

CLASS_VALUES: list = [i for i in range(6)] # default max classes = 6
...
class Composite(object):
    ...
    def generate_single_grid(
                ...
                overwrite: bool = False, #*MW added to enable overwriting of .zarrs
                fill_value: int = 10,
                burn_area_value: int = BURN_AREA_VALUE, #*MW passing through config now
        ...
        if overwrite:
            self._logger.info(f'OVERWRITE zarrs is on')
        ...
        #*MW added enable overwrite zarr and delete zarr path if it already exists
        if os.path.exists(tile_path) and not overwrite: 
            self._logger.info(f'{tile}- {tile_path} already exists or overwrite off')
            return xr.open_zarr(tile_path)
        elif os.path.exists(tile_path) and overwrite: # delete zarr to avoid overwrite err - this will delete entire group not just dataset which is fine for here
            shutil.rmtree(tile_path)
            
        tile_df = self._grid[self._grid['tile'] == tile]
        strips = self._model_output[self._model_output['tile'] == tile]
        ...
        if tile_df.empty:
            self._logger.info(f'Tile {tile} is not in grid file')
            return None
        ...
        concat_array = xr.concat(arrays, dim='time', fill_value=fill_value) #*MW
        concat_array.rio.write_nodata(fill_value)
        concat_dataset = concat_array.to_dataset(name=name, promote_attrs=True) #* ensure attributes eg FillValue are written to zarr
        
        del arrays, concat_array
        
        if write_out:
            self._logger.info(f'{tile} - Writing ({n_strips} strips) to {tile_path}')
    ...
    def calculate_binary_class_stats(stat: str,
        ...
        try:
            ...
            #import pdb; pdb.set_trace()
            logger.info(f'Could not process {output_path} due to NoData input array values')
            return None
        # max_class = tile_ndarray[tile_ndarray != nodata].max() #*TD get this from config - maybe some tiles have max_class=6, others 4
    ...
    ############################################################################
    #*MW TD are the functions below used anymore?
    ############################################################################
    ...
    ############################################################################
    # NO LONGER USED FUNCTIONS ? Need to check #*MW TODO
    ############################################################################
...
#*MW TEMP DELETE
import psutil
def print_memory_usage(top_level_only=True):
    # Get the memory usage of the current process
    process = psutil.Process()  # Current process
    memory_info = process.memory_info()
    
    # Convert to MB
    memory_used = memory_info.rss / (1024 ** 3)  # RSS (resident set size)
    
    
    # Get system-wide memory stats
    memory = psutil.virtual_memory()
    memory_used2 = memory.used / (1024 ** 3)  # RSS (resident set size)
    
    memory_available = memory.available / (1024 ** 3)  # Available memory (in MB)
    memory_total = memory.total / (1024 ** 3)  # Total system memory (in MB)
    
    print(f"  Memory used by current process: {memory_used:.2f} / {memory_used2:.2f} GB")
    if not top_level_only:
        print(f"  Available / Total system memory: {memory_available:.2f} / {memory_total:.2f} GB")
    # print(f"  Memory available: {memory_available:.2f} GB\n")
    ...
    if not top_level_only:
       ...
       # Get the total number of CPUs
        ...
        # Initialize memory usage tracking for each CPU
        cpu_memory_usage = {cpu: 0 for cpu in range(num_cpus)}
        
        # Iterate through all processes
        for proc in psutil.process_iter(['pid', 'memory_info', 'cpu_affinity', 'name']):
            try:
                # Get the memory used by the process (in bytes)
                mem_usage = proc.info['memory_info'].rss  # Resident Set Size (physical memory used)
                # Get the CPUs the process is bound to
                cpu_affinity = proc.info['cpu_affinity']
                
                # Distribute memory usage across CPUs in the process's affinity
                if cpu_affinity:
                    per_cpu_memory = mem_usage / len(cpu_affinity)
                    for cpu in cpu_affinity:
                        cpu_memory_usage[cpu] += per_cpu_memory
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                # Skip processes that are no longer active or restricted
                continue
        
        # Print results
        print("\nMemory usage per CPU (approximation):")
        for cpu, mem in cpu_memory_usage.items():
            if int(mem) >= 1073741824:
                print(f"   CPU {cpu}: {mem / (1024**3):.2f} GB", end=";  ")
    
    
        # $ try %
         # Get number of logical CPUs (threads)
        logical_cpus = num_cpus
        # Get number of physical CPUs (cores)
        physical_cpus = psutil.cpu_count(logical=False)
    
        # Get CPU usage percentage per logical CPU
        ...
        print(f"\n  Logical CPUs (threads) / Physical CPUs (cores): {logical_cpus} / {physical_cpus}")
        # print(f"Physical CPUs (cores): {physical_cpus}")
        print("\n   CPU Usage per CPU core (percentage):")
        for i, usage in enumerate(cpu_usage_per_cpu):
            if int(usage) > 10:
                print(f"     CPU {i}: {usage}%", end=";  ")
        print()